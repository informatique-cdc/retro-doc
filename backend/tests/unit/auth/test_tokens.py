...
class TestCreateAccessToken:
    """Mint a short-lived access token from a set of identity claims."""

    def test_access_token_round_trip(self, token_claims: TokenClaims) -> None:
...
class TestCreateRefreshToken:
    """Mint a longer-lived refresh token from a set of identity claims."""

    def test_refresh_token_round_trip(self, token_claims: TokenClaims) -> None:
...
class TestDecodeInternalToken:
    """Verify an app-issued token's signature, claims and declared type."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                create_access_token,
                REFRESH_TOKEN_TYPE,
                "Expected token of type 'refresh'",
                id="access-as-refresh",
            ),
            pytest.param(
                create_refresh_token,
                ACCESS_TOKEN_TYPE,
                "Expected token of type 'access'",
    ...
    def test_decode_rejects_tampered_setting(
        ...
        """A token valid at issue time is rejected when a security-critical setting differs."""
        token = create_access_token(token_claims)
        monkeypatch.setattr(auth_settings, setting, value)

        with pytest.raises(exception):
            decode_internal_token(token, expected_type=ACCESS_TOKEN_TYPE)

    def test_alg_none_rejected(self, token_claims: TokenClaims) -> None:
...
class TestUserUidRoundTrip:
    """Preserve a user's stable identifier across an app-issued token."""

    def test_uid_is_stable_across_token_round_trip(self) -> None:
